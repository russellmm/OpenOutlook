...
internal static class PstCrypto
{
    private static readonly byte[] MpbbCrypt = BuildPermuteTable();
    private static readonly uint[] Crc32 = BuildCrc32Table();

    public static void Decode(Span<byte> data, PstCryptMethod method, uint cyclicKey)
    {
        switch (method)
        {
            case PstCryptMethod.None:
                return;
            case PstCryptMethod.Permute:
                Permute(data, encrypt: false);
                return;
            case PstCryptMethod.Cyclic:
                Cyclic(data, cyclicKey);
                return;
            case PstCryptMethod.WipEncrypted:
                throw new PstException("This PST is protected with Windows Information Protection and cannot be opened.");
            default:
                throw new PstException($"Unsupported PST encoding method 0x{(byte)method:X2}.");
        }
    }

    public static void Encode(Span<byte> data, PstCryptMethod method, uint cyclicKey)
    {
        switch (method)
        {
            case PstCryptMethod.None:
    ...
    public static uint ComputeCrc(ReadOnlySpan<byte> data, uint seed = 0)
    {
        var crc = seed;
    ...
    public static ushort ComputeSig(ulong ib, ulong bid)
    {
        var x = ib ^ bid;
        return (ushort)((ushort)(x >> 16) ^ (ushort)x);
    }

    private static void Permute(Span<byte> data, bool encrypt)
    {
        var table = encrypt ? MpbbCrypt.AsSpan(0, 256) : MpbbCrypt.AsSpan(512, 256);
        for (var i = 0; i < data.Length; i++)
            data[i] = table[data[i]];
    }

    private static void Cyclic(Span<byte> data, uint key)
    {
        var r = MpbbCrypt.AsSpan(0, 256);
        var s = MpbbCrypt.AsSpan(256, 256);
        var inv = MpbbCrypt.AsSpan(512, 256);
    ...
    private static byte[] BuildPermuteTable() =>
    [
